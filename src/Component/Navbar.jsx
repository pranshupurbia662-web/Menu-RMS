    ...
    <div className="w-full bg-[#f7f3ef] border-b border-[#D4A017]/30 shadow-sm">
      ...
      <div className="flex items-center justify-between px-3 py-2">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h1 className="text-xl sm:text-3xl font-semibold text-[#111827]">Royal Spice</h1>
            <p className="text-[10px] sm:text-sm tracking-[3px] sm:tracking-[5px] text-[#D4A017] uppercase">Fine Dining</p>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-5 lg:gap-8 text-[14px] lg:text-[17px] font-medium uppercase text-[#D4A017]">
          {links.map((l) => (
            <Link key={l.to} to={l.to} className="hover:text-[#B8860B] transition duration-300">
              {l.label}
            </Link>
          ))}
        </div>
        ...
        >
          <span className={`block w-6 h-0.5 bg-[#D4A017] transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-[7px]' : ''}`}></span>
          <span className={`block w-6 h-0.5 bg-[#D4A017] transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}></span>
          <span className={`block w-6 h-0.5 bg-[#D4A017] transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-[7px]' : ''}`}></span>
        </button>
      </div>
